...
interface Props {
  ...
  /** When set, renders the ✕ close button and closes after a successful sell. */
  onClose?: () => void;
  onSell?: (instanceId: string) => Promise<void>;
}

// The detail "panel" itself (animated GIF sprite + full info + sell), with no
// backdrop. PokemonDetailModal wraps a single one in a full-screen backdrop;
// PokemonStackModal renders several side by side in a grid.
export default function PokemonDetailCard({ pokemon, onClose, onSell }: Props) {
